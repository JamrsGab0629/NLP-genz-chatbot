...
# A handful of entries in the dataset are old-school ALL-CAPS chat acronyms
# that happen to be spelled exactly like everyday English function words once
# lowercased (e.g. the acronym "WAS", "SO", "AT", "WE", "ARE"). If those were
# matched case-insensitively, the bot would flag nearly every normal sentence
# ("I WAS at the store" -> "was" and "at" both light up). Everything else in
# the dataset — including well-known slang that happens to be stored in caps
# like "GOAT" or "NPC" — should match no matter how the user types it, since
# that's the whole point (people mostly type slang in lowercase).
AMBIGUOUS_ACRONYMS = {"was", "so", "at", "we", "are"}
...
def _resolve_entries(key, matched_text):
    ...
    if not entries:
        return None
    if key in AMBIGUOUS_ACRONYMS and not matched_text.isupper():
        # only trust an exact ALL-CAPS shout for these five; lowercase is
        # almost certainly just the ordinary English word, not the acronym
        return None
    return entries
...
# NOTE: templates use a colon/dash before {desc} rather than folding it into
# a lowercase sentence continuation ("...that means {desc}"). Descriptions in
# the dataset are inconsistently capitalized (some are plain phrases, some
# are acronym expansions like "Live Action Role Play" or "Greatest of all
# Times"), so a colon reads naturally either way without any text-mangling.
SINGLE_SLANG_TEMPLATES = [
    'Oh, "{term}"? That means: {desc}. Example: {example}',
    '"{term}" = {desc}. Kinda like: "{example}"',
    'Ha, caught that — "{term}" means: {desc}.',
    'You said "{term}"! That\'s slang for: {desc}.',
    'Fun fact: "{term}" — {desc}. {context}',
...
def build_slang_reply(hits):
    ...
    for term, entry in hits:
        if term.lower() in seen:
            continue
        seen.add(term.lower())
        lines.append(MULTI_SLANG_LINE.format(term=term, desc=entry["description"]))
    return "\n".join(lines)
...
def random_slang_fact():
    ...
    return f'Here\'s a random one: "{entry["raw"]}" — {entry["description"]}. e.g. "{example}"'
